...
describe("keyboard e2e", () => {
  for (const term of TERMS) {
    test(`toggles the sidebar via leader shortcut in ${term}`, async () => {
    ...
    test(`opens the selected sidebar file with keyboard navigation in ${term}`, async () => {
      await withLatestSessionDiagnostics(sessionStack, async () => {
        const session = await startSession(term, {
          "a.md": "# a\n",
          "b.md": "# b\n",
    ...
    test(`returns typing to the editor after leaving sidebar focus in ${term}`, async () => {
      await withLatestSessionDiagnostics(sessionStack, async () => {
        const session = await startSession(term, {
          "note.md": "# note\n",
        })
    ...
    test(`cancels the unsaved-quit prompt with escape in ${term}`, async () => {
      await withLatestSessionDiagnostics(sessionStack, async () => {
        const session = await startSession(term, {
          "note.md": "# note\n",
        })

...
describe("prompt and file-flow e2e", () => {
  for (const term of TERMS) {
    test(`blocks editor input while the unsaved prompt is visible in ${term}`, { timeout: 20000 }, async () => {
    ...
    test(`opens the pending file after choosing Don't Save in ${term}`, { timeout: 20000 }, async () => {
    ...
    test(`saves an untitled buffer through Save As before continuing in ${term}`, { timeout: 20000 }, async () => {
      const { fixture, session } = await startSession(term, {})
